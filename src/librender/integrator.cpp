...
MTS_VARIANT bool
LightTracerIntegrator<Float, Spectrum>::render(Scene *scene, Sensor *sensor) {
    ScopedPhase sp(ProfilerPhase::Render);
    m_stop = false;
...
LightTracerIntegrator<Float, Spectrum>::sample_visible_emitters(
    ...
    // if (ek::any(active_e)) {  // TODO
        ...
        // auto infinite_emitters = ek::select(active_e, emitter, nullptr);
    ...
    // if (ek::any(active_e)) {  // TODO
        // // TODO: do we really need this?
        // auto finite_emitters = ek::select(active_e, emitter, nullptr);
        // Log(Info, "active_e = %s", ek::count(active_e));
        // Log(Info, "finite_emitters = %s",
        //     ek::count(ek::neq(finite_emitters, nullptr)));
...
MTS_VARIANT double LightTracerIntegrator<Float, Spectrum>::normalize_film(
