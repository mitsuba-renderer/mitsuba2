 ...
 * - filename
   - |string|
   - Filename of the ABC file that should be loaded 
 * - shape_index
   - |int|
...
public:
    ...
    AlembicMesh(const Properties &props) : Base(props) {
        ...
        const uint shape_index = props.int_("shape_index", 0);
        ...
        // how do we create multiple mesh shapes?
        // for now create just one shape specified by shape_index
        if (shape_index > (uint) archive_top.getNumChildren() - 1)
            fail(tfm::format("Unable to load mesh. Shape index \"%i\" is "
                                "out of range 0..%i",
                                shape_index, archive_top.getNumChildren() - 1));

        IObject object = archive_top.getChild(shape_index);
        ScalarTransform4f obj_xform = read_abc_xform(object);
		find_children_recursively(object, sample_index, alembic_transform, alembic_polymeshes);

        if (alembic_polymeshes.empty())
            fail("found no polygonal meshes in file.");
			...
			if (!m_disable_vertex_normals && polymesh_has_vertex_normals)
			{
                ...
                for (size_t i = 0; i < normals.size(); i++)
                {
                    ...
                    if ( unlikely(!all(enoki::isfinite(n))) ){
                        Log(Warn, "Invalid vertex normal: %s at vertex %s." , n, i );
                    }
                    normals[i] = n;
                }
			}
...
private:
            ...
            const PropertyHeader & property_header )
    {
        ...
        ISampleSelector sample_selector( m_alembic_time ); // alembic_time, for now just use value at 0 time
        typename geom_param_t::sample_type param_sample;
        param.getExpanded(param_sample, sample_selector);

        // 3 for color; 1 for shop_materialpath
        ...
        switch (param_sample.getScope())
        {
            ...
            case kVertexScope:
                ...
                add_attribute(attr_name, extent, FloatStorage::copy(values, m_vertex_count * extent));
                break;
            ...
            case kUniformScope:{
                ...
                if(is_face_attr){
                    add_attribute(attr_name, extent, FloatStorage::copy(values, m_face_count * extent));
                }
                else{
    ...
    void find_children_recursively(const IObject& obj,
		...
		ScalarTransform4f &t, std::vector<AlembicPolymesh>& map)
	{
		for (size_t i = 0; i < obj.getNumChildren(); i++)
		{
			...
			if ( IXform::matches(child.getMetaData()) ){
				...
				polymesh.xform = t;
                if (!m_load_visible || GetVisibilityProperty(child) == (bool)kVisibilityVisible){
                    map.push_back(polymesh);
                }
                else if (GetVisibilityProperty(child) == (bool)kVisibilityHidden){
                    // Issue a message in the log for invisible object perhaps
                }                       
				continue;
			}
            // continue traversing alembic tree for more child objects
			find_children_recursively(child, sample_index, child_transform, map);
		}
	}
