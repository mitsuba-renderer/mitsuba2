...
public:
    ...
    SurfaceInteraction3f fill_surface_interaction(const Ray3f &ray,
        ...
        si.dp_du[active] = m_to_world * si.dp_du * (2.f * math::Pi<Float>);
        si.dp_dv[active] = m_to_world * si.dp_dv * math::Pi<Float>;
    ...
    std::pair<Point3f, Normal3f> differentiable_position(const SurfaceInteraction3f &si,
                                                         Mask /*active*/) const override {
        Float phi   = si.uv.x() * math::TwoPi<Float>,
              theta = si.uv.y() * math::Pi<Float>;

        auto [s_theta, c_theta] = sincos(theta);
        auto [s_phi, c_phi] = sincos(phi);

        Point3f p = Point3f(s_theta * c_phi, s_theta * s_phi, c_theta);
        Normal3f n = p;

        return { m_to_world.transform_affine(p), m_to_world.transform_affine(n) };
    }
